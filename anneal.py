...
class Dataset(data.Dataset):
  ...
  def __getitem__(self, q):
        ...
        #N = [0]
        H = np.load("Processed/"+ID+"/"+str(N)+".npy")
...
if __name__=='__main__':
  ...
  model.prior.requires_grad=False
  ...
  trainset = Dataset(1)
  trainloader = torch.utils.data.DataLoader(trainset, batch_size=B, shuffle=False, num_workers=2, pin_memory=False) #TODO TRAINSETs
  ...
  for epoch in range(epochs*subepochs):
    for q, Data in enumerate(trainloader, 0):
          ...
          mse = 256**2*model.SLoss(pred, I) #+256**2/(epoch+1)*(pred.std()-I.std())**2
          #hl = 100*model.HLoss(pred,I)
          kl = 5*model.KLoss

          #if mse.mean().item() < 800:
          #  pass
          ...
          #adS = 100*(((Gx-gx)**2).mean(-1).mean(-1).mean(-1) + ((Gy-gy)**2).mean(-1).mean(-1).mean(-1))
          loss = 0*(mse + kl).mean()
          ...
          print("means", model.gaussian_parameters(model.prior)[0].min().item(),  model.gaussian_parameters(model.prior)[0].max().item(), "std", "({} {})".format(np.round(pred.std().item(), 3), np.round(I.std().item(), 3)))#, "grads", adS.mean().item())
          if q == 0:
              losses.append([kl.mean().item(), mse.mean().item()])
  ...
  torch.save(model.state_dict(), 'Modelanneal.pth')
