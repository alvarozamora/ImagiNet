...
class VAE(nn.Module):
    def __init__(self):
        ...
        self.HLoss1 = pytorch_ssim.DSSIM(window_size = 16, size_average = True)
        #self.HLoss2 = pytorch_ssim.DSSIM(window_size = 64, size_average = True)
        self.SLoss = nn.MSELoss()

    def Encoder(self):
        return nn.Sequential(nn.Conv2d(   3,  32, 5, 1, 0), nn.LeakyReLU(),
                             nn.Conv2d(  32,  64, 5, 2, 0), nn.LeakyReLU(),
                             nn.Conv2d(  64, 128, 5, 2, 0), nn.LeakyReLU(), 
                             nn.Conv2d( 128, 256, 5, 2, 0), nn.LeakyReLU(), 
                             nn.Conv2d( 256, 512, 5, 2, 0), nn.LeakyReLU(),
                             nn.Conv2d( 512, 512, 5, 2, 0), nn.LeakyReLU(), Flatten(),

                             nn.Linear(8192, 1024), nn.LeakyReLU(),
    ...
    def Decoder(self):
        return nn.Sequential(nn.Linear(self.h_dim, self.h_dim), nn.LeakyReLU(),
                             ...
                             nn.Linear(1024, 8192), nn.LeakyReLU(), Reshape((-1, 4, 4)),
                             ...
                             nn.Conv2d(32, 32, 3, 1, 0), nn.LeakyReLU(),
                             nn.Upsample(scale_factor=(2,2), mode='bilinear'), 
                             ...
                             nn.Conv2d(16, 16, 3, 1, 0), nn.LeakyReLU(),
                             nn.Upsample(scale_factor=(2,2), mode='bilinear'),
                             nn.Conv2d(16, 8, 3, 1, 0), nn.LeakyReLU(),
                             nn.Conv2d(8, 3, 1, 1, 0))
...
if __name__=='__main__':
  ...
  #model = torch.load('Model_all_85.pth')
  #model.prior = torch.load('Model_prior_85.pth')
  load = False
  ...
  #optimizer.load_state_dict(torch.load('Optim_85.pth'))
  ...
  subepochs = 1
  for epoch in range(85, epochs*subepochs):
    for q, Data in enumerate(trainloader, 0):
      for iolimloop in range(3):
      #if q in [1,2]:
          ...
          if mse.mean().item() < 400:
            ...
            kl = torch.max(10000*torch.ones_like(kl), kl)
          ...
          print("means", model.gaussian_parameters(model.prior)[0].min().item(),  model.gaussian_parameters(model.prior)[0].max().item(), "std", "({} {})".format(np.round(pred.std(dim=(-2,-1)).mean().item(), 3), np.round(I.std(dim=(-2,-1)).mean().item(), 3)), "DSSIM", hl.mean().item())
          if model.iter%100 == 0:
